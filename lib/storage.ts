...
export function clearAllTemplates(): void {
  ...
  console.log("Cleared all templates from storage")
}
  ...
  try {
    ...
    for (const t of templates) {
      try {
        ...
        console.error(`Skipping corrupted template: ${t.name || t.id}`, error)
        // Skip this corrupted template and continue with others
      }
    }

    if (validTemplates.length < templates.length) {
      console.log(`Removed ${templates.length - validTemplates.length} corrupted template(s)`)
    ...
    console.error("Error reading templates, clearing storage:", error)
    clearAllTemplates()
    return []
  }
}
